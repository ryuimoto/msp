...
@endsection
@section('item_name')
  <a class="navbar-brand" href="#pablo">全ユーザーポイント集計</a>
@endsection
@section('contents')
  <div class="row">
                ...
                <div id="typography">
                  <div class="row">
                    <div class="col-md-9 offset-md-3">
                      <div class="tim-typo">
                        <h2>
                          <span class="tim-note">総合ポイント</span>{{ $points_amount }}P</h2>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <tbody>
                @forelse ($users as $user)
                  <tr role="row" class="odd">
                    <td tabindex="0" class="sorting_1"><a id="big" href="{{ route('admin.points_individual_total',['user_id' => $user->user->id ]) }}">{{ $user->user->id }}</a></td>
                    <td><a id="big" href="{{ route('admin.points_individual_total',['user_id' => $user->user->id]) }}">{{ $user->user->name }}</a></td>
                    <td><a id="big" href="{{ route('admin.points_individual_total',['user_id' => $user->user->id]) }}">{{ $user->point }}</a></td>
                  </tr>
                @empty
                @endforelse
              </tbody>
...
@section('js')
  <script>
    $(function(){
      $("#points,#points_total").addClass('active');
    });
  </script>
@endsection